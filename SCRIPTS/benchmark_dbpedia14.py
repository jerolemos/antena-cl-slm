#!/usr/bin/env python3
"""
ANTENA Mega-Test Final — DBPedia14 + Inteligencia Zero-Shot
=============================================================================
Este script implementa la "Ruta A" (Esfuerzo Leve) de la Suite Final.
"""
...
# Las dependencias ahora se manejan externamente en el Cuaderno de Colab.
...
try:
    ...
    print("Warning: Importando desde directorio local de Colab...")
    from antena_gestalt_core import EGPRRegulator, LayerGestalt
    from antena_latent_il2a import MidLayerGestalt, compute_il2a_kl_loss
    from antena_paso2_lora_ogp import LoRALinear, LoRAGestalt, inject_lora, remove_lora
...
# =============================================================================
# 2. HERRAMIENTAS ZERO-SHOT Y LENGUAJE
# =============================================================================
...
def evaluate_intelligence(classification_model, tokenizer):
    ...
    del causal_model; gc.collect(); torch.cuda.empty_cache()
...
# 4. CARGA DBPEDIA
# =============================================================================
def prepare_dbpedia(tokenizer):
    print("[INFO] Cargando DBPedia14...")
    ds = load_dataset("fancyzhx/dbpedia_14")
    def tokenize(batch): return tokenizer(batch['content'], truncation=True, padding='max_length', max_length=MAX_LENGTH)
    train_datasets, test_datasets = [], []
    for task_id in range(NUM_TASKS):
        classes = [task_id * 2, task_id * 2 + 1]
        t = ds['train'].filter(lambda x: x['label'] in classes).shuffle(seed=42)
        t = t.select(range(min(SAMPLES_PER_TASK, len(t)))).map(tokenize, batched=True)
        t.set_format(type='torch', columns=['input_ids', 'attention_mask', 'label'])
        train_datasets.append(t)
        
        te = ds['test'].filter(lambda x: x['label'] in classes).shuffle(seed=42)
        te = te.select(range(min(SAMPLES_PER_TASK // 2, len(te)))).map(tokenize, batched=True)
...
def calc_aia(R, max_t): return np.mean([np.mean([R[i][t] for i in range(t+1)]) for t in range(max_t)])

def save_checkpoint(results, variant_name=None, r_matrix=None):
    if not results: return
    df = pd.DataFrame(results)
    df.to_csv(f"{RESULTS_DIR}/mega_test_dbpedia_results.csv", index=False)
    ...
    if os.path.exists("/content/drive/MyDrive"):
        ...
        df.to_csv(f"{drive_path}/mega_test_dbpedia_results.csv", index=False)
        if variant_name and r_matrix is not None:
            np.savetxt(f"{drive_path}/{filename}", r_matrix, delimiter=",")
    log_summary(f"💾 Checkpoint guardado ({len(results)} variantes)")
...
# =============================================================================
# 5. MOTOR DE ENTRENAMIENTO DBPEDIA
# =============================================================================
def run_dbpedia_model(tokenizer, train_datasets, test_datasets, cfg, mode_name):
    ...
    rho, lwf_w = cfg.get('rho', 0.99), cfg.get('lwf', 5.0)
    ...
    # Inyectar LoRA (adaptación confinada — reemplaza el catastrófico Full Fine-Tuning)
    ...
    # Hook para calibrar la covarianza de la Capa 10 (Mid-Layer)
    ...
    for t in range(NUM_TASKS):
        ...
        if t > 0:
            model_frozen = copy.deepcopy(model).eval()
        ...
        for ep in range(EPOCHS_PER_TASK):
            for i, b in enumerate(loader):
                ...
                loss_lwf, loss_mem = torch.tensor(0.0, device=device), torch.tensor(0.0, device=device)
                ...
                if t > 0:
                    ...
                    if replay_sz > 0 and replay_buf:
                        rb = replay_buf.sample(inputs.size(0))
                        if rb: loss_mem = loss_mem + model(rb['input_ids'].to(device), attention_mask=rb['attention_mask'].to(device), labels=rb['label'].to(device)).loss

                    plasticity = egpr.compute(outputs.logits.detach())
                    loss_total = loss_task + lwf_w * loss_lwf + lwf_w * loss_mem
                    loss_total.backward()
                    with torch.no_grad():
                        for g in gestalts.values():
                            g.project_lora_a_grad()
                    opt.param_groups[0]['lr'] = lora_lr * plasticity
                    opt.param_groups[1]['lr'] = score_lr * plasticity
                else: loss_task.backward()
                ...
                if i % 5 == 0:
                    ...
                    if t > 0:
                        ...
                        extra = f" | LwF:{loss_lwf.item():.4f} | P:{plasticity:.3f} | Loss_Ratio:{loss_ratio:.4f}"
                    print(f"  [Ep {ep}] Batch {i}/{len(loader)} | Loss Task: {loss_task.item():.4f}{extra}")

        # PHASE 3: Weight Aligning (post-task bias correction)
        if t > 0:
            with torch.no_grad():
                old_c = torch.arange(t * 2)
                new_c = torch.arange(t * 2, (t + 1) * 2)
        ...
        if use_il2a:
            ...
            for lbl in range(class_start, class_end):
                if lbl in il2a_augmentors: il2a_augmentors[lbl].fit()

        if replay_sz > 0: replay_buf.add_task(loader.dataset)
...
def run_variant_worker(nombre, cfg, t_tr, t_te, tokenizer, queue, sem):
    with sem:
        ...
        try:
            trained_model, aia, bwt, R = run_dbpedia_model(tokenizer, t_tr, t_te, cfg, nombre)
            ...
            log_summary(f"🌟 {nombre}: PIQA={scores['PIQA']:.2f}% | PPL={scores['Perplexity']:.4f}")
...
if __name__ == "__main__":
    ...
    print("="*70)
    print("ANTENA MEGA-TEST: DBPEDIA14 + PIQA (Simetría) - PARALELO")
    print("="*70)
    ...
    t_tr, t_te = prepare_dbpedia(tokenizer)
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                champs = {
                    "Zero-Exemplar": {"rho": 0.99, "lwf": 5.0, "use_il2a": False, "il2a_temp": 0.0, "replay_sz": 0, "lora_lr": 5e-4, "score_lr": 1e-4},
                    "IL2A": {"rho": 0.99, "lwf": 5.0, "use_il2a": True, "il2a_temp": 1.0, "replay_sz": 0, "lora_lr": 5e-4, "score_lr": 1e-4},
                    "TinyReplay": {"rho": 0.99, "lwf": 5.0, "use_il2a": False, "il2a_temp": 0.0, "replay_sz": 100, "lora_lr": 5e-4, "score_lr": 1e-4}
                }

    completed_variants = []
    final_results = []
    if os.path.exists(f"{RESULTS_DIR}/mega_test_dbpedia_results.csv"):
        print("[INFO] Checkpoint encontrado en RESULTS/. Reanudando ejecución...")
        df_prev = pd.read_csv(f"{RESULTS_DIR}/mega_test_dbpedia_results.csv")
    ...
    print("\n=======================================================")
    print("🏆 RESULTADOS FINALES DE DBPEDIA (PAPER)")
    print("=======================================================")
    df = pd.DataFrame(final_results)
    print(df.to_string(index=False))
    
    os.system(f"zip -r final_dbpedia_output.zip {RESULTS_DIR}/")
