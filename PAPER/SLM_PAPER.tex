...
To address this challenge, \textbf{ANTENA-CIL} is evaluated, a buffer-free class-incremental learning framework engineered specifically for sub-150M parameter models. ANTENA-CIL operates without storing historical text samples by decomposing structural protection into two complementary levels:
...
\textbf{Orthogonal LoRA Methods.} O-LoRA \citep{wang2023orthogonal} enforces orthogonality between task-specific LoRA subspaces to prevent interference. InfLoRA \citep{liang2024inflora} designs interference-free LoRA adaptation subspaces. Both target larger models (LLaMA-7B, T5-Large); ANTENA-CIL addresses the sub-150M regime where parameter budget constraints are more severe.

\textbf{Exemplar-Free Class-Incremental Learning.} Learning Without Forgetting \citep[LwF;][]{li2017learning} uses knowledge distillation on new data as a proxy for old knowledge. IL2A---Class-Incremental Learning via Dual Augmentation \citep{zhu2021class}---introduces class augmentation for representation bias and semantic augmentation via stored class statistics for classifier bias. ANTENA-CIL adopts IL2A's semantic augmentation strategy but replaces full covariance Cholesky sampling with SVD-filtered signal subspace sampling to reduce noise.
...
% ══════════════════════════════════════════════════════════════
\section{ANTENA-CIL Framework Architecture}
...
\begin{equation}
\mathbf{G}_{\text{protected}} = \mathbf{G}(\mathbf{I} - \rho \, \mathbf{U}_{\text{sig}} \mathbf{U}_{\text{sig}}^T)
\end{equation}
...
\midrule
\textbf{ANTENA-CIL\_Base} ($\lambda_{\text{LwF}}{=}5.0$) & \textbf{82.88\%} & +29.17\% & 64.53\% & 172.65 & \textbf{0} \\
\textbf{ANTENA-CIL\_SlowDistill} ($\lambda_{\text{LwF}}{=}7.5$) & 80.67\% & \textbf{+38.37\%} & \textbf{64.58\%} & \textbf{101.79} & \textbf{0} \\
...
Configuration & AIA $\uparrow$ & BWT $\uparrow$ & PIQA $\uparrow$ & PPL $\downarrow$ & Buffer \\
\midrule
\textbf{ANTENA-CIL\_Base} ($\lambda_{\text{LwF}}{=}5.0$) & 63.49\% & $-$58.24\% & 56.37\% & 813,633 & \textbf{0} \\
\textbf{ANTENA-CIL\_SlowDistill} ($\lambda_{\text{LwF}}{=}7.5$) & \textbf{73.69\%} & \textbf{$-$31.03\%} & \textbf{59.25\%} & \textbf{20,731} & \textbf{0} \\
...
\textbf{ANTENA-CIL (Soft-OGP)} & 67.20\% & 67.00\% & $-$0.20\% \\
\bottomrule
\end{tabular}
\end{table}

Both EWC and ANTENA-CIL exhibit minimal reasoning variation ($\Delta = -0.20\%$), indicating that Soft Orthogonal Gradient Projection maintains out-of-domain reasoning manifolds during targeted parameter updates.
...
In edge computing networks, user privacy prevents uploading raw conversation transcripts to a central server. Under ANTENA-CIL, edge devices share only local covariance matrices $\boldsymbol{\Sigma}_i$ or principal eigenvectors $\mathbf{U}_i$---abstract geometric manifolds containing \textbf{zero raw text tokens}.
...
\begin{enumerate}
    ...
    \item \textbf{Generative Perplexity on Long Sequences.} While ANTENA-CIL preserves backbone perplexity on DBpedia14 (7 tasks, PPL~172.65), the CLINC-150 benchmark (15 tasks) reveals substantial perplexity degradation even with SlowDistill (PPL~20,731 vs.\ baseline~172), demarcating the practical limit of perplexity preservation for the current architecture. Mitigating long-sequence generative degradation remains an open challenge.
